...
const createSeeMiddleware = () => {
  ...
  const publishRelationDependencies = async (noDependencies, cache, filterCache, channel, req, streamFilters, element) => {
    ...
    if (fromStix && toStix) {
      ...
      const isFromVisible = await isInstanceAccessible(user, fromStix, filterCache, streamFilters);
      const isToVisible = await isInstanceAccessible(user, toStix, filterCache, streamFilters);
        ...
        for (let index = 0; index < elements.length; index += 1) {
          ...
          if (isCompatibleVersion) {
            ...
            if (!isInferredData || (isInferredData && withInferences)) {
              const isCurrentlyVisible = await isInstanceAccessible(user, stix, filterCache, streamFilters);
              if (type === EVENT_TYPE_UPDATE) {
                const { newDocument: previous } = jsonpatch.applyPatch(R.clone(stix), context.reverse_patch);
                const isPreviouslyVisible = await isInstanceAccessible(user, previous, filterCache, streamFilters);
