  ...
  const handleIndicatorUpsert = async (indicator: StixIndicator): Promise<Array<Event>> => {
    ...
    if (!revoked) {
      ...
      for (let index = 0; index < sightingsRelations.length; index += 1) {
        ...
        const incidentToIndicatorCreation = await createInferredRelation(incidentToIndicator, ruleRelContent) as RelationCreation;
        ...
        const incidentToIdentityCreation = await createInferredRelation(incidentToIdentity, ruleRelContent) as RelationCreation;
